...
contract DaoFactory {
    ...
    daoInfo[] public daos;
    mapping(address => address[]) public daoUsers;
    function createDao(uint parliamentCount, uint approveCount, uint workDays, uint applyTime, uint applyMemberCount, uint depositCount, uint voteInterval, uint publicityTime, string memory name,address govToken
        ...
        daoUsers[address(dao)].push(msg.sender);
        return (address(dao));
    }
    ...
    function joinDao(address daoAddress) public {
        ...
        daoUsers[daoAddress].push(msg.sender);
    }
    function getDaoUserLength(address daoAddress) public view returns(uint){
        return daoUsers[daoAddress].length;
    }
